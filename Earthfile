...
lint-migrate:
  ...
  RUN --mount=type=cache,target=/root/.cache/pip pip install sqlfluff==$sqlfluff_version
  WORKDIR migrate
...
test-integration:
  ...
  WITH DOCKER --compose compose.yaml --service mysql --pull migrate/migrate:v$migrate_version --pull golang:$go_version
    RUN \
